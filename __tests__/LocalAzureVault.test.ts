...
describe('Read properties from Azure.', () => {
  test('should use chart to load secrets', async () => {
    ...
    const theConfig = await properties.addFromAzureVault(testConfig, { pathToHelmChart: '__tests__/chart/values.yaml' })
  ...
  test('should be able to use actual config module as expected', async () => {
    await properties.addFromAzureVault(config, { pathToHelmChart: '__tests__/chart/values.yaml' })
  ...
  test('should throw correct exception if the chart does not exist', () => {
    expect(async () => await properties.addFromAzureVault(config, { pathToHelmChart: '__tests__/chart/does-not-exist.yaml' }))
      .rejects
      .toThrowError("helm chart not found at: '__tests__/chart/does-not-exist.yaml'")
  })
