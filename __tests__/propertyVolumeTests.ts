...
describe('Read properties from files on a mount or folder.', () => {
  test('should start with secrets read from volume', () => {
    ...
    const theConfig = properties.addTo(testConfig, {
      mountPoint: '__tests__/testVolumes/secrets',
    });
  ...
  test('should ignore sub directories', () => {
    const testConfig: any = {};
    properties.addTo(testConfig, {
      mountPoint: '__tests__/testVolumes/secrets-sub-dir',
    });
